...
This is the framework-agnostic *reference implementation* for the FluidSim
project. Its job is correctness, not speed: it establishes ground-truth physics
(validated against published wind-tunnel / DNS values) that the future GPU
solver port (Taichi / Warp / CUDA -- TBD by tech-stack research) must reproduce.
...
# Direction-index groups by sign of cx, used for the inlet/outlet BCs.
COL_PLUS = np.array([0, 1, 2])      # cx = +1  (rightward)
COL_ZERO = np.array([3, 4, 5])      # cx =  0
COL_MINUS = np.array([6, 7, 8])     # cx = -1  (leftward)
...
class LBM2D:
    ...
    def __init__(self, nx: int, ny: int, *, u_lb: float, re: float,
        ...
        self.vel = np.zeros((2, nx, ny))
        self.vel[0] = u_lb * (1.0 + 5e-3 * np.sin(
            np.linspace(0, 2 * np.pi, ny)[None, :].repeat(nx, axis=0)))

        # Distribution functions f_i(x, y); start at equilibrium of the inflow.
        self.f = self._equilibrium(np.ones((nx, ny)), self.vel)
        self._boundary_force = np.zeros(2)    # last computed [Fx, Fy] (lattice)
    ...
    def _equilibrium(self, rho: np.ndarray, u: np.ndarray) -> np.ndarray:
        ...
        feq = np.empty((9, self.nx, self.ny))
        for i in range(9):
    ...
    def step(self) -> None:
        ...
        for i in range(9):
            self.f[i] = np.roll(np.roll(fout[i], C[i, 0], axis=0),
                                C[i, 1], axis=1)
    ...
    def _momentum_exchange_force(self, fout: np.ndarray) -> np.ndarray:
        ...
        for i in range(9):
            ...
            neigh_body = np.roll(np.roll(body, -C[i, 0], axis=0),
                                 -C[i, 1], axis=1)
    ...
    def coefficients(self) -> tuple[float, float]:
        ...
        return self._boundary_force[0] / denom, self._boundary_force[1] / denom

    def vorticity(self) -> np.ndarray:
        """Scalar out-of-plane vorticity dvy/dx - dvx/dy (for visualisation)."""
        _, u = self.macroscopic()
        dvy_dx = np.gradient(u[1], axis=0)
        dvx_dy = np.gradient(u[0], axis=1)
        w = dvy_dx - dvx_dy
        w[self.solid] = np.nan          # blank the body in plots
        return w
